...
#### vim中的GPS
	...
	上面就是定标记位，那如何找到标记位呢？只要你知道你已经打了那些标记位，那你就可以通过`标记位号
	来跳到你想要到的标记位上面去。
...
" 是最近的删除或者复制存放的寄存器
+ 是与系统共享的寄存器.随着你用vim的时间增加,你可能会发现在系统中其它地方复制的东西是无法直接用p,P粘贴上去的.这时你要用 "+p 或者"+P
...
另外:如果你对vim的其它命令熟悉,你完全可以自定义你想要的快捷键.(以后会有介绍的)



## 1.9 有规则的修改
	
	1.9.1 给批量的字加前缀或者后缀 (块的操作)
