...
####vim的文件保存与vim的退出####
	...
	:q						  // 为quit的缩写，这是退出vim
	:w +[filename]			  // w 为write的缩写, filename为你要保存的文件名。如果有了文件名，则可以不写,如果写了一个新的文件名，则为别存为。
	:wq						  // 保存并且退出
	:x +[filename]			  // 保存并且退出,与wq唯一的区别是当文件不更改时不会更新修改时间,同样如有文件名可以不写
	:q!						  // 强制不保存退出。
	:w!						  // 强制保存
	:wa						  // 保存全部 (a为all的缩写)
	:qa						  // 全部退出
	...
	:wqa					  // 全部保存退出
	:wqa!					  // 强制全部保存退出
	:28,100sw +[filename]	  // 28行到100行保存到filename中
	ZZ						  // 相当于wq
...
1.6.3. 复制。
	 ...
	 yy                          复制光标所在的行
	 Y                	      	 复制光标所在的行
	 :[rang]co+adress      		 rang复制的行范围,co为copy,adress要复制到的目的地
...
_另外_ :如果你对vim的其它命令熟悉,你完全可以自定义你想要的快捷键.(以后会有介绍的)
